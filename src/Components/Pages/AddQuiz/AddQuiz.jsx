...
export const AddQuiz = () => {
  ...
  const [userCon1, setUserCon] = useState("");
  const [userCon2, setUserCon2] = useState("");
  ...
  const handlePdfGenerateQuiz = () => {
    getPdfGeneratedQuiz(file, userCon1, userCon2).then((res) => {
      if (res.array) {
  ...
  const handleGenerateQuiz = () => {
    ...
    //   alert(`Please enter a minimum of ${textLength} characters`);
    // } else {
    setLoading(true);
    getGeneratedQuiz(text, noQuiz)
      .then((res) => {
        if (res.array) {
          setQuestions(res?.array);
        } else {
  ...
  return (
    <>
      <div className="flex sm:flex-row flex-col  rounded-lg shadow-lg mx-4   bg-white   gap-4 border-2 sm:p-6 sm:m-6 p-1">
        <form
        ...
        <div className="text-gray-500 h-full md:w-1/2 w-full    p-6 ">
          <div className="flex justify-center items-center flex-col gap-y-3">
            ...
            <textarea
              ...
              rows="7"
              cols="50"
              placeholder={`Enter text minimum  500 characters`}
            />
      ...
      <div className="flex    h-80  rounded-lg shadow-lg mx-4   bg-white   gap-4 border-2 sm:p-6 sm:m-6 p-1">

        <div className="flex flex-col h-full overflow-y-scroll md:w-1/2 w-full px-5  text-black no-scrollbar gap-2">
          <h2 className="text-sm uppercase text-center">Previously added Quiz</h2>
          {quizs ? (
            quizs?.map((item, index) => (
              <div
                key={item?._id}
                className=" rounded-lg bg-slate-50 flex justify-between items-center border-2 p-2"
              >
                <div className="flex justify-between text-xs sm:text-lg relative items-center w-full">
                  ...
                  <div className="flex items-center group/item   gap-5">
                    ...
                    <div
                      id="dropdown"
                      className=" z-[99999] group/item group-hover/item:visible invisible block right-0 top-9 absolute  bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700"
                    >
                      <ul
        ...
        <div className="flex flex-col h-full overflow-y-scroll md:w-1/2 w-full px-5  text-black no-scrollbar gap-2">
          <div className="">
            ...
            <input
              onChange={handleFileChange}  // Use a separate function to handle file changes
              className="block w-full py-2 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none"
              id="file_input"
              type="file"
            />
          </div>
          <input
            ...
            placeholder="Correct Answer Index (comma separated)"
            value={userCon1}
            onChange={(e) => setUserCon(e.target.value)}
            required
            className="w-full bg-slate-50 p-3 input input-bordered border-black  mb-2 text-black"
          />
          <input
            ...
            placeholder="Correct Answer Index (comma separated)"
            value={userCon2}
            onChange={(e) => setUserCon2(e.target.value)}
            required
            className="w-full bg-slate-50 p-3 input input-bordered border-black  mb-2 text-black"
          />
          <button
            onClick={handlePdfGenerateQuiz}
            className="relative inline-flex items-center w-fit justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800"
          >
            <span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
              {loading && (
                <svg
